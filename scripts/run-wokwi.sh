...
if [ "${WOKWI_PROJECT_ID}" == "" ]; then
    wokwi-server --chip esp32s3 target/${ESP_ARCH}/${BUILD_MODE}/esp32-s3-usb-otg
else
    wokwi-server --chip esp32s3 --id ${WOKWI_PROJECT_ID} target/${ESP_ARCH}/${BUILD_MODE}/esp32-s3-usb-otg
fi